...
const UserCartItemsContent = function UserCartItemsContent({ cartItem }) {
  ...
  const handleQuantityChange = useCallback((newQuantity) => {
    ...
    // Find the product for inventory validation
    const product = productList.find(p => p._id === cartItem.productId);
    if (!product) {
      toast({
    ...
    if (selectedColor) {
      ...
      const colorInventory = selectedColor.inventory || 0;
      availableInventory = colorInventory;
    ...
    if (newQuantity > availableInventory) {
      if (availableInventory === 0) {
        toast({
          title: selectedColor ? 'Selected color is out of stock' : 'This product is out of stock',
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: `Only ${availableInventory} item${availableInventory > 1 ? 's' : ''} available for this ${inventorySource}`,
        variant: "destructive",
      });
      return;
    }
    ...
    // Proceed with quantity update after validation
    dispatch(
      updateCartQuantity({
        userId: user?.id,
        productId: cartItem?.productId,
  ...
  }, [cartItem, selectedColor, user, dispatch, toast, productList]);
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex justify-between items-start w-full">
            <h3 className={`text-sm font-medium mb-1 line-clamp-2 pr-6 ${themeColors.cardText}`}>{cartItem?.title}</h3>
          ...
          <div className="flex justify-between items-center mb-2">
            <p className={`text-sm ${themeColors.cardText}`}>
              {formatCurrency(cartItem?.salePrice > 0 ? parseFloat(cartItem.salePrice) : parseFloat(cartItem?.price || 0))}
              {cartItem?.quantity > 1 && <span className={themeColors.mutedText}> × {cartItem?.quantity}</span>}
            </p>
            <p className={`text-sm font-medium ${themeColors.cardText}`}>
              {formatCurrency((cartItem?.salePrice > 0 ? parseFloat(cartItem.salePrice) : parseFloat(cartItem?.price || 0)) *
